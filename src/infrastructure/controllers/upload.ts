...
export function uploadManager(req: Request, res: Response) {
  const path: string = req.query.path as string;
  const mode: string = req.query.mode as string; // req.query is an object with a property for every QueryParameter existing in URL. That's why I'm guessing it can only be string and number
  const content: string = req.query.content as string;

  switch (path) {
    case "/upload":
      directUpload(mode, content);
      res.send(
        `Single file upload using direct routefinished. Mode: ${mode}. Content: ${content}.`,
      );
      break;
    case "/uploads":
      const paramUploadFileName: string = req.query.uploadFileName as string;
      const bodyUploadFileName: string = req.body.uploadFileName as string;

      fromRequestUpload(mode, content, paramUploadFileName, bodyUploadFileName);
      res.send("Single file upload from request finished");
      break;
    default:
      console.error("Invalid path parameter");
      res.status(400).send("Upload manager: Invalid path parameter");
  }
}

function directUpload(mode: string, content: string) {
...
export function fromRequestUpload(
  mode: string,
  content: string,
  paramUploadFileName: string,
  bodyUploadFileName: string,
) {
  ...
  pipeline(
    fs.createReadStream(`../../../uploads/${bodyUploadFileName}.${mode}`), // This file is overwritten continuously, no matter if it's authors or books
    csv(),
    transformChunk(content),
    fs.createWriteStream("/dev/null"), // .createWriteStream() recieves string or Buffer types.
    (err) => {
      ...
      if (err) {
        ...
        console.log(
          "End of upload pipeline() using file and route from request",
        );
      }
    },
  );
...
function transformChunk(content: string): Transform {
